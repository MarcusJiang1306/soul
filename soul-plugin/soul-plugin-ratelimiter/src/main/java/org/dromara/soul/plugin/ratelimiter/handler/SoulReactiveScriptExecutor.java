...
public class SoulReactiveScriptExecutor<K> extends DefaultReactiveScriptExecutor<K> {
    ...
    private <T> Flux<T> execute(final ReactiveRedisCallback<T> action) {
        ...
        return Flux.usingWhen(Mono.fromSupplier(factory::getReactiveConnection), action::doInRedis, ReactiveRedisConnection::closeLater).doOnError(throwable -> log.error("Redis execute exception: {}", throwable.getMessage()));
    }
}
